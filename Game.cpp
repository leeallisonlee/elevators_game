...
void Game::playGame(bool isAIModeIn, ifstream& gameFile) {
    ...
    initGame(gameFile);
        ...
        while (gameFile >> temp) {
            Person temp2(temp);
            while (temp2.getTurn() > building.getTime()) {
...
bool Game::isValidPickupList(const string& pickupList, const int pickupFloorNum) const {
    ...
    //each element is non negative digit
    ...
    //length of pickupList <= elevator capacity
    if (pickupList.size() > ELEVATOR_CAPACITY) {
        return false;
    }
    
    //list size < number of people on floor
    char tempChar = pickupList[0];
    for (int i = 0; i < pickupList.size(); i++) {
        if (pickupList[i] > tempChar) {
            tempChar = pickupList[i];
        }
    }
    if ((tempChar - '0') >= building.getFloorByFloorNum(pickupFloorNum).getNumPeople()) {
        return false;
    }
    //everyone in pickupList must be going the same direction
    for (int i = 0; i < pickupList.size() - 1; i++) {
        for (int j = i + 1; j < pickupList.size(); j++) {
            if (((building.getFloorByFloorNum(pickupFloorNum).getPersonByIndex(pickupList[i] - '0').getTargetFloor()) > pickupFloorNum) && ((building.getFloorByFloorNum(pickupFloorNum).getPersonByIndex(pickupList[j] - '0').getTargetFloor()) < pickupFloorNum)) {
                return false;
            } else if (((building.getFloorByFloorNum(pickupFloorNum).getPersonByIndex(pickupList[i] - '0').getTargetFloor()) < pickupFloorNum) && ((building.getFloorByFloorNum(pickupFloorNum).getPersonByIndex(pickupList[j] - '0').getTargetFloor()) > pickupFloorNum)) {
                return false;
            }
        }
    }
